...
import com.pulsebeat02.shoeraffleservice.application.privacy.SuspicousLogin;
...
import com.pulsebeat02.shoeraffleservice.application.session.Session;
...
public class LoginPanel {
    ...
    // public Loading loadingFrame = new Loading();
    ...
    public void start(boolean makeNew) {
	...
	if (!makeNew) {
	    ...
	    ShoeRaffleService.service.getInstanceManager().LOGIN_PANEL = new LoginPanel();
	    ShoeRaffleService.service.getInstanceManager().LOGIN_PANEL.frmLogin.setVisible(true);
	...
	frmLogin.setIconImage(Toolkit.getDefaultToolkit().getImage(ShoeRaffleService.class
	...
	lblUserLogin.setIcon(new ImageIcon(LoginPanel.class
		.getResource("/com/pulsebeat02/shoeraffleservice/resources/images/login/userLogin.png")));
	...
	lblNewLabel_2.setIcon(new ImageIcon(
		LoginPanel.class.getResource("/com/pulsebeat02/shoeraffleservice/resources/images/login/user.png")));
	...
	label.setIcon(new ImageIcon(
		LoginPanel.class.getResource("/com/pulsebeat02/shoeraffleservice/resources/images/login/lock.png")));
	...
	Login.addMouseListener(new MouseAdapter() {
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			Thread loginThread = new Thread(() -> {
				...
				ShoeRaffleService.service.getInstanceManager().MANAGE_SESSIONS.allSessions.put(session.location.IP, session);
			    } catch (IOException e) {
				e.printStackTrace();
			    }
			...
			try {
			    ShoeRaffleService.service.getInstanceManager().MANAGE_THREADS.addThread(loginThread,
				    ShoeRaffleService.getInstance().cpuThreads);
			} catch (Exception e) {
			    e.printStackTrace();
			    System.exit(0);
			}
	...
	btnRegister.addMouseListener(new MouseAdapter() {
		...
		frmLogin.dispose();
		ShoeRaffleService.service.getInstanceManager().REGISTER_PANEL = new RegisterPanel();
	    }
	});
	...
	ImageIcon imageIcon = new ImageIcon(
		LoginPanel.class.getResource("/com/pulsebeat02/shoeraffleservice/resources/images/login/login.png"));
	...
	lblNewLabel_1.addMouseListener(new MouseAdapter() {
	    ...
	    @Override
	    public void mouseClicked(MouseEvent e) {
		ShoeRaffleService.service.getInstanceManager().RESET_PASSCODE = new ResetPasscode();
	    }
	});
	...
	if (ShoeRaffleService.instance == null) {
	    System.out.println(true);
	}
	
	ShoeRaffleService.service.getInstanceManager().MANAGE_THREADS = new ManageThreads();
